ComboKeyboard,蓝牙/USB按键宏
支持中文
提供宏编辑器
多宏文件储存功能、多主题切换、宏文件储存功能(读取一次SD卡后自动将宏文件储存在内部)
系统保留按键FN 和上下左右
上下切换亮度
左右切换xml宏文件
FN+R重新读取SD卡
FN+H暂停页面切换
FN+G/J 单个xml内部翻页
FN+左右切换主题
FN+D切换delay模式，DLY:ON按照xml宏文件内部定义的触发时间(例如xml内delay=15是15ms后触发下一个动作)，DLY:OFF固定delay=15ms。
ComboKeyboard, Bluetooth/USB Key Macro
Supports Chinese
Provides a macro editor
Multi-macro file storage, multi-theme switching, macro file storage (automatically stores macro files internally after reading the SD card once)
System reserved keys: FN and arrow keys (up, down, left, right)
Up/down to adjust brightness
Left/right to switch XML macro files
FN+R to re-read the SD card
FN+H to pause page switching
FN+G/J for page navigation within a single XML file
FN+Left/Right to switch themes
FN+D to toggle delay mode: DLY:ON follows the trigger time defined in the XML macro file (e.g., delay=15 in XML means 15ms before triggering the next action), DLY:OFF fixes the delay at 15ms.
